...
class ArenaViewSet(ViewSet):
    ...
    def fulfill(self, request):
        """
        Fulfill hackathon winner payout to blockchain
        ...
        # Get best prediction by avg error
    ...
    def players(self, request):
        """
        Get list of players for a hackathon
        """
        # Validate hackathon_id parameter
        try:
            hackathon_id = int(request.query_params.get("hackathon_id", 0))
        except (TypeError, ValueError):
            logger.error("Invalid hackathon_id parameter")
            return Response({"error": "Invalid hackathon_id parameter"}, status=400)
        
        if hackathon_id <= 0:
            logger.error("hackathon_id must be a positive integer")
            return Response({"error": "hackathon_id must be a positive integer"}, status=400)

        try:
            players = get_players(hackathon_id)
            logger.info(f"Retrieved {len(players) if players else 0} players for hackathon {hackathon_id}")
            return Response({
            ...
            logger.error(f"Failed to get players for hackathon {hackathon_id}: {str(e)}")
            return Response({"error": f"Failed to retrieve players: {str(e)}"}, status=500)